...
class SimulationConfig:
    ...
    # Economy parameters
    initial_balance: float = 0.5       # FIL seed capital per agent
...
def main():
    ...
    print(f"Total rewards paid: {metrics.rewards_paid[-1]:.4f} FIL")
    print(f"Total penalties: {metrics.penalties_collected[-1]:.4f} FIL")
    ...
    for strategy in config.agent_strategies:
        ...
        print(f"  {strategy:15s} | survived={survived} | earned={earned:.4f} FIL | final_tier=T{tier}")
    ...
    print(f"\n--- Theorem 2 Check ---")
    print(f"  Adaptive earned:   {adaptive_earned:.4f} FIL")
    print(f"  Aggressive earned: {aggressive_earned:.4f} FIL")
