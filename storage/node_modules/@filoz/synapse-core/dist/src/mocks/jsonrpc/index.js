...
import { bytesToHex, decodeFunctionData, encodeAbiParameters, isAddressEqual, multicall3Abi, numberToBytes, numberToHex, parseEther, parseUnits, stringToHex, toHex, } from 'viem';
import * as Piece from "../../piece.js";
import { TIME_CONSTANTS } from "../../utils/constants.js";
import { ADDRESSES } from "./constants.js";
import { endorsementsCallHandler } from "./endorsements.js";
import { erc20CallHandler } from "./erc20.js";
import { paymentsCallHandler } from "./payments.js";
import { pdpVerifierCallHandler } from "./pdp.js";
import { serviceProviderRegistryCallHandler } from "./service-registry.js";
import { sessionKeyRegistryCallHandler } from "./session-key-registry.js";
import { warmStorageCallHandler, warmStorageViewCallHandler } from "./warm-storage.js";
export { ADDRESSES, PRIVATE_KEYS, PROVIDERS } from "./constants.js";
function jsonrpcHandler(item, options) {
    const { id } = item;
    try {
        return {
            jsonrpc: '2.0',
...
export function JSONRPC(options) {
    return http.post('https://api.calibration.node.glif.io/rpc/v1', async ({ request }) => {
...
function handler(body, options) {
    ...
    switch (method) {
        ...
        case 'eth_call': {
            ...
            if (isAddressEqual(ADDRESSES.calibration.warmStorage, to) ||
                isAddressEqual(ADDRESSES.mainnet.warmStorage, to)) {
                return warmStorageCallHandler(data, options);
            }
            if (isAddressEqual(ADDRESSES.calibration.multicall3, to)) {
                return multicall3CallHandler(data, options);
            }
            if (isAddressEqual(ADDRESSES.calibration.spRegistry, to)) {
                return serviceProviderRegistryCallHandler(data, value, from, options);
            }
            if (isAddressEqual(ADDRESSES.calibration.sessionKeyRegistry, to)) {
                return sessionKeyRegistryCallHandler(data, options);
            }
            if (isAddressEqual(ADDRESSES.calibration.endorsements, to) ||
                isAddressEqual(ADDRESSES.mainnet.endorsements, to)) {
                return endorsementsCallHandler(data, options);
            }
            if (isAddressEqual(ADDRESSES.calibration.viewContract, to)) {
                return warmStorageViewCallHandler(data, options);
            }
            if (isAddressEqual(ADDRESSES.calibration.pdpVerifier, to)) {
                return pdpVerifierCallHandler(data, options);
            }
            if (isAddressEqual(ADDRESSES.calibration.payments, to)) {
                return paymentsCallHandler(data, options);
            }
            if (isAddressEqual(ADDRESSES.calibration.usdfcToken, to) ||
                isAddressEqual(ADDRESSES.customToken, to)) {
                return erc20CallHandler(data, options);
            }
...
function multicall3CallHandler(data, options) {
...
export const presets = {
    basic: {
        ...
        eth_sendRawTransaction: (args) => {
            const deserialized = TxEnvelopeEip1559.deserialize(args[0]);
        ...
        warmStorageView: {
            ...
            getDataSetMetadata: (args) => {
                ...
                if (dataSetId === 1n && key === 'withCDN')
                    return [true, ''];
                if (dataSetId === 1n && key === 'environment')
                    return [true, 'test'];
                return [false, ''];
            },
            ...
            getPieceMetadata: (args) => {
                ...
                if (dataSetId === 1n && pieceId === 0n && key === 'withIPFSIndexing')
                    return [true, ''];
