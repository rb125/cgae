...
import { bytesToHex, createWalletClient, numberToHex, http as viemHttp, } from 'viem';
...
describe('StorageService', () => {
    ...
    describe('create() factory method', () => {
        it('should select a random provider when no providerId specified', async () => {
            ...
            assert.isTrue(service.serviceProvider === Mocks.PROVIDERS.provider1.providerInfo.serviceProvider ||
        ...
        it('should skip existing datasets and return -1 with providerId when forceCreateDataSet is true', async () => {
            ...
            assert.equal(context.serviceProvider, Mocks.PROVIDERS.provider1.providerInfo.serviceProvider, 'Should select the requested provider');
        ...
        it('should prefer data sets with existing pieces', async () => {
            ...
            server.use(Mocks.JSONRPC({
                ...
                warmStorageView: {
                    ...
                    getDataSet: (args) => {
                        const [dataSetId] = args;
                        return [expectedDataSets.find((ds) => ds.dataSetId === dataSetId) ?? {}];
                    },
                },
            }), Mocks.PING({
                baseUrl: Mocks.PROVIDERS.provider1.products[0].offering.serviceURL,
    ...
    describe('preflightUpload', () => {
        it('should calculate costs without CDN', async () => {
            ...
            const preflight = await service.preflightUpload({ size: Number(SIZE_CONSTANTS.MiB) });
        ...
        it('should handle insufficient allowances', async () => {
            ...
            const warmStorageService = new WarmStorageService({ client });
            const service = await StorageContext.create({ synapse, warmStorageService, withCDN: true });
            const preflight = await service.preflightUpload({ size: Number(100n * SIZE_CONSTANTS.MiB) });
        ...
        it('should enforce minimum size limit in preflightUpload', async () => {
            ...
            const service = await StorageContext.create({ synapse, warmStorageService, withCDN: true });
            try {
                await service.preflightUpload({ size: 126 });
    ...
    describe('download', () => {
        it('should download and verify a piece', async () => {
            ...
            server.use(Mocks.JSONRPC({
                ...Mocks.presets.basic,
            }), Mocks.PING(), http.get(`https://${Mocks.ADDRESSES.client1}.calibration.filbeam.io/:cid`, async () => {
                return HttpResponse.text('Not Found', {
                    status: 404,
                });
            }), Mocks.pdp.findPieceHandler(testPieceCID, true, pdpOptions), http.get('https://pdp.example.com/piece/:pieceCid', async () => {
        ...
        it('should handle download errors', async () => {
            ...
            server.use(Mocks.JSONRPC({
                ...Mocks.presets.basic,
            }), Mocks.PING(), Mocks.pdp.findPieceHandler(testPieceCID, true, pdpOptions), http.get('https://pdp.example.com/piece/:pieceCid', async () => {
    ...
    describe('upload', () => {
        it('should handle errors in batch processing gracefully', async () => {
            server.use(Mocks.JSONRPC({
                ...Mocks.presets.basic,
            }), Mocks.PING(), http.post('https://pdp.example.com/pdp/piece', async () => {
                return HttpResponse.error();
        ...
        it.skip('should handle transaction failure on-chain', async () => {
            ...
            }), Mocks.pdp.uploadPieceHandler(mockUuid, pdpOptions), http.get('https://pdp.example.com/pdp/piece', async () => {
                return HttpResponse.json({ pieceCid: testPieceCID });
            }), Mocks.pdp.createAndAddPiecesHandler(mockTxHash, pdpOptions), http.get('https://pdp.example.com/pdp/data-sets/created/:tx', async () => {
        ...
        it('should handle upload piece failure', async () => {
            ...
            server.use(Mocks.JSONRPC({
                ...Mocks.presets.basic,
            }), Mocks.PING(), Mocks.pdp.postPieceHandler(testPieceCID, mockUuid, pdpOptions), http.put('https://pdp.example.com/pdp/piece/upload/:uuid', async () => {
                return HttpResponse.error();
            }));
            const synapse = new Synapse({ client });
            const warmStorageService = new WarmStorageService({ client });
        ...
        it('should handle add pieces failure', async () => {
            ...
            server.use(Mocks.JSONRPC({
                ...Mocks.presets.basic,
            }), Mocks.PING(), Mocks.pdp.postPieceUploadsHandler(mockUuid, pdpOptions), Mocks.pdp.uploadPieceStreamingHandler(mockUuid, pdpOptions), Mocks.pdp.finalizePieceUploadHandler(mockUuid, undefined, pdpOptions), Mocks.pdp.findPieceHandler(testPieceCID, true, pdpOptions), http.post('https://pdp.example.com/pdp/data-sets/:id/pieces', () => {
                return HttpResponse.error();
            }));
            const synapse = new Synapse({ client });
            const warmStorageService = new WarmStorageService({ client });
    ...
    describe('pieceStatus()', () => {
        ...
        it('should return exists=false when piece not found on provider', async () => {
            server.use(Mocks.JSONRPC({
                ...Mocks.presets.basic,
            }), Mocks.PING(), http.get('https://pdp.example.com/pdp/data-sets/:id', async () => {
                return HttpResponse.json({
                    id: 1,
                    pieces: [],
    ...
    describe('getPieces', () => {
        it('should get all active pieces with pagination', async () => {
            ...
            server.use(Mocks.PING(), Mocks.JSONRPC({
                ...Mocks.presets.basic,
                pdpVerifier: {
                    ...Mocks.presets.basic.pdpVerifier,
