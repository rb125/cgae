...
class LLMAgent:
    ...
    def __init__(self, model_config: dict):
        ...
        self.deployment_name: str = model_config["deployment_name"]
        ...
        if not self._api_key:
            raise EnvironmentError(
                f"Missing env var {api_key_var} for model {self.model_name}"
            )
        if not self._endpoint:
            raise EnvironmentError(
                f"Missing env var {endpoint_var} for model {self.model_name}"
            )
